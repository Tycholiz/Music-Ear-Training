...
export function ChordsScreen({
  ...
  const toggle = (id: string, checked: boolean) => {
    ...
    const next = checked
      ? CHORDS.filter((chord) => enabled.has(chord.id) || chord.id === id).map(
          (chord) => chord.id,
        )
  ...
  return (
    <div className="flex flex-col gap-6 p-4">
      {CHORD_CATEGORIES.map((category, index) => (
        <ListCard
          key={category}
          title={category}
          // Attach the warning to the last card so it reads as a summary of
          // the whole screen rather than of one category.
          footer={index === CHORD_CATEGORIES.length - 1 ? warning : undefined}
        >
          {chordsInCategory(category).map((chord) => {
